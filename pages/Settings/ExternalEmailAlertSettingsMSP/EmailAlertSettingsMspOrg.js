...
// Sub page
import EmailAlertSettingsMspOrgToolBarBox from './EmailAlertSettingsMspOrgToolBarBox';
import EmailAlertSettingsMspOrgTable from './EmailAlertSettingsMspOrgTable';
...
const defaultModalStatus = {
  addUser: {
    label: '3846c46b9e', // Add users
    status: false,
    disabled: false,
  },
  delete: {
    label: 'f2a6c498fb', // delete,
    description: '9a33a28fd4', // Would you like to delete selected?
    status: false,
    disabled: false,
  },
  resend: {
    label: 'b451caa3da', // Resend Invitation
    status: false,
    disabled: false,
  }
};
...
const GateWaySettings = props => {
  const { alertSettings, settingsKey } = props;
  const { t } = useTranslation();
  let gatewayItem = null;

  if (settingsKey === 'notifyGatewayOffline') {
    gatewayItem = (
      <div className={`text-nowrap ${mainStyle['delay-container']}`}>
        <div>{t(alertSettings[settingsKey].label)}</div>
        <DropdownWithItem
          selectedItem={alertSettings[settingsKey].delayList[0]}
          itemList={alertSettings[settingsKey].delayList}
          onClick={e => { }}
        />
        <div>{t(alertSettings[settingsKey].unit)}</div>
      </div>
    )
  } else if (settingsKey === 'notifyGatewayNetworkUsageExceed') {
    gatewayItem = (
      <div className={mainStyle['usage-container']}>
        <div className='text-nowrap'>{t(alertSettings[settingsKey].label)}</div>
        <Input
          type='number'
          placeholder='1-999'
          value={alertSettings[settingsKey].quantity}
          onChange={() => { }}
          onFocus={() => { }}
          onBlur={() => { }}
        />
        <DropdownWithItem
          selectedItem={alertSettings[settingsKey].unitList[0]}
          itemList={alertSettings[settingsKey].unitList}
          onClick={e => { }}
        />
        <div className='mx-1'>{t('ba8d2b9408')}</div> { /* in */}
        <DropdownWithItem
          selectedItem={alertSettings[settingsKey].timeRangeList[0]}
          itemList={alertSettings[settingsKey].timeRangeList}
          onClick={e => { }}
        />
      </div>
    )
  } else {
    gatewayItem = <div className='text-nowrap'>{t(alertSettings[settingsKey].label)}</div>
  }

  return gatewayItem;
}

const EmailAlertSettingsMspOrg = () => {
  const { t } = useTranslation();
  ...
  return (
    <div className={`layout-container layout-container--column layout-container--fluid ${mainStyle['alert-settings-container']}`}>
      ...
      <EmailAlertSettingsMspOrgToolBarBox
        modalStatus={modalStatus}
        changeModalStatus={changeModalStatus}
      />

      <EmailAlertSettingsMspOrgTable
        modalStatus={modalStatus}
        changeModalStatus={changeModalStatus}
      />
                ...
                <tbody>
                  {
                    Object.keys(alertSettings).map(key => {
                      return (
                        ...
                        <tr key={alertSettings[key].id.tr}>
                          <td key={alertSettings[key].id.title} >
                            <GateWaySettings {...{ alertSettings, settingsKey: key }} />
                          </td>
...
export default EmailAlertSettingsMspOrg;
