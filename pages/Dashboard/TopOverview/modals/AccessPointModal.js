...
import img_device_gateway from 'assets/img/v2/icon/device_ap.png';
      ...
      <tbody>
        {
          deviceList.map((device, index) => {
            return <tr key={nanoid()}>
              ...
              <td><a href={`#/cloud/configure/access-point/device/${index}`}>{device.name}</a></td>
...
const AccessPointModal = props => {
  const { modalStatus, changeModalStatus } = props;
  const { t } = useTranslation();

  // Fake API data
  const fakeDeviceList = [
    { ...generateDevice('AP', 'DBA-1210P', 'AP001', 'Online') },
    { ...generateDevice('AP', 'DBA-2720P', 'AP002', 'Offline', null, 'Active') }
  ];
  ...
  return (
    <div className=''>
      <ModalContainer
        ...
        openModal={modalStatus.ap.status}
        closeModal={() => changeModalStatus('ap', false)}
      >
        <div className={`header ${topOverviewStyle['align-place-header']}`}>
          <div>
            <img className='mx-3' src={img_device_gateway} alt="" width={65} />
          </div>
          <div className="title">Access points</div>
        </div>
        <div className='body'>
          <AccessPointList deviceList={deviceList} setDeviceList={setDeviceList} />
