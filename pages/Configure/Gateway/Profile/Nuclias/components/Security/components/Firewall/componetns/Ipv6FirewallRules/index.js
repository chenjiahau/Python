...
const Ipv6FirewallRules = () => {
  ...
  // Side effect
  useEffect(() => {
    if (!firewallState?.ipv6FirewallRules) return
    const updatedWans = [];
    const updatedVlans = [];
    ...
    for (const rule of firewallState?.ipv6FirewallRules) {
      if (rule.isSample) {
  ...
  }, [firewallState?.ipv6FirewallRules]);
  ...
  return (
    <>
      <Func title='IPV6 FIREWALL RULES'>
        <div className='mb-2'>
          <div className='justify-content-between d-flex'>
            ...
            <InlineTitle isNonUnderline>
              ...
              <DropdownWithCheckbox
                id='ipv6-firewall-rules-field-dropdown'
                type='checkbox'
                          ...
                          !isUseProfileConfig ? (
                            <>
                              <div className='d-flex'>
                                <RadioButton
                                  id={`ipv6-firewall-rules-active-${index}-enable`}
                                  name={`ipv6FirewallRulesActive${index}`}
                                  label='Enable'
                                  hasRightMargin={true}
                                  checked={item.active}
                                ...
                                />
                                <RadioButton
                                  id={`ipv6-firewall-rules-active-${index}-disable`}
                                  name={`ipv6FirewallRulesActive${index}`}
                                  label='Disable'
                                  checked={!item.active}
...
export default Ipv6FirewallRules;