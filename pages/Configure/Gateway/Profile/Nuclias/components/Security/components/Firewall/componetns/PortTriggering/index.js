...
const PortTriggering = () => {
  ...
  // Side effect
  useEffect(() => {
    setList(cloneDeep(firewallState.portTriggering));
    setProfileList(cloneDeep(firewallState.portTriggering));
  }, [firewallState.portTriggering]);
          ...
          >
            <thead>
              <tr>
                ...
                <th>
                  <LinkerWithA
                    label='Outgoing trigger port'
                    className='text-decoration-none'
                    onClick={e => {
                      isUseProfileConfig && sorting(e, profileList, 'outgoingTriggerPort', setProfileList);
                      !isUseProfileConfig && sorting(e, list, 'outgoingTriggerPort', setList);
                    }}
                  />
                </th>
                <th>
                  <LinkerWithA
                    label='Incoming trigger port'
                    className='text-decoration-none'
                    onClick={e => {
                      isUseProfileConfig && sorting(e, profileList, 'incomingTriggerPort', setProfileList);
                      !isUseProfileConfig && sorting(e, list, 'incomingTriggerPort', setList);
                    }}
                  />
                </th>
                <th>
                ...
                sourceList.map((item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>{item.name}</td>
                      <td>{item.protocol}</td>
                      <td>{item.outgoingTriggerPort}</td>
                      <td>{item.incomingTriggerPort}</td>
                      <td className='input'>
                        {
                          !isUseProfileConfig ? (
                            <>
                              <div className='d-flex'>
                                <RadioButton
                                  id={`port-triggering-active-${index}-enable`}
                                  name={`portTriggeringActive${index}`}
                                  label='Enable'
                                  hasRightMargin={true}
                                  checked={item.active}
                                ...
                                />
                                <RadioButton
                                  id={`port-triggering-active-${index}-disable`}
                                  name={`portTriggeringActive${index}`}
                                  label='Disable'
                                  checked={!item.active}
