...
const EditModal = props => {
  ...
  useEffect(() => {
    ...
    const portForwardingConfig = cloneDeep(selectedConfig.config);
    ...
    modeDefinition.forEach(item => {
      modeDropdown.push({
        title: item,
        value: item,
        isActive: item === portForwardingConfig.mode
      });
    });
    ...
    protocolDefinition.forEach(item => {
      protocolDropdown.push({
        title: item,
        value: item,
        isActive: item === portForwardingConfig.protocol
      });
    });
    ...
    interfaceDropdown.forEach(item => {
      if (item.value === portForwardingConfig.interface) {
        item.isActive = true;
      }
    });
  ...
  return (
    ...
    >
      <div className='header'>
        <div className='title'>Edit port forwarding</div>
      </div>
      <div className='body'>
        {/* Name, Mode*/}
        <Row className='mt-2'>
