...
const EditModal = props => {
  ...
  useEffect(() => {
    ...
    const ipv6FirewallRuleConfig = cloneDeep(selectedConfig.config);
    ...
    policyDefinition.forEach(item => {
      policyDropdown.push({
        title: item,
        value: item,
        isActive: item === ipv6FirewallRuleConfig.policy
      });
    });
    ...
    protocolDefinition.forEach(item => {
      protocolDropdown.push({
        title: item,
        value: item,
        isActive: item === ipv6FirewallRuleConfig.protocol
      });
    });
    ...
    sourceInterfaceDropdown.forEach(item => {
      if (item.value === ipv6FirewallRuleConfig.sourceInterface) {
        item.isActive = true;
      }
    });
    ...
    schedulePolicies.forEach(item => {
      schedulePolicyDropdown.push({
        title: item.title,
        value: item.title,
        isActive: item.title === ipv6FirewallRuleConfig.schedule
      });
    });
  ...
  return (
    ...
    >
      <div className='header'>
        <div className='title'>Edit IPv6 firewall rules</div>
      </div>
      <div className='body'>
        {/* Priority, Name*/}
        <Row className='mt-2'>
