...
const EditConfigModal = props => {
  ...
  useEffect(() => {
    ...
    const ripConfig = cloneDeep(selectedConfig.config);
    ...
    interfaceList.forEach(iface => {
      ...
      interfaceDropdown.push({
        ...
        isActive: ripConfig.interface === iface.interface
      });
    });
    ...
    directionDefinition.forEach(direction => {
      directionDropdown.push({
        ...
        isActive: ripConfig.direction === direction.value
      });
    });
    ...
    versionDefinition.forEach(version => {
      versionDropdown.push({
        title: version.title,
        value: version.value,
        isActive: ripConfig.version === version.value
      });
    });
  ...
  return (
    ...
    >
      <div className='header'>
        <div className='title'>Edit RIP configuration</div>
      </div>
      <div className='body'>
        {/* Interface, Direction */}
        <Row className='mt-2'>
