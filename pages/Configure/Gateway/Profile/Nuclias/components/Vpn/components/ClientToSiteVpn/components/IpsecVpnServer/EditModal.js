...
import { generateEditIpsecVpnServer } from 'dummy/data/gateway/data/vpn/client-to-site-vpn/ip-sec-vpn-server';
...
const EditModal = ({
  ...
  useEffect(() => {
    const tmpIke = generateEditIpsecVpnServer(selectedIpsecVpnServer);
    setIpsecVpnServer(tmpIke);
  }, [modalStatus.editIpSecVpnServer.status])

  if(!ipsecVpnServer) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.editIpSecVpnServer.status}
      closeModal={() => changeModalStatus('editIpSecVpnServer', false)}
    >
      <div className='header'>
        <div className='title'>Edit IPsec VPN server</div>
      </div>
      <div className='body'>
      ...
      <div className='footer'>
        <Button
          ...
          onClick={() => changeModalStatus('editIpSecVpnServer', false)}
        />
