...
import { getAddIkeProfiles } from 'dummy/data/gateway/data/vpn/client-to-site-vpn/ike-profiles';
...
const AddModal = ({
  ...
  useEffect(() => {
    const tmpIke = getAddIkeProfiles();
    setIkeProfile(tmpIke);
  }, [modalStatus.addIke.status])

  if(!ikeProfile) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-750px'
      openModal={modalStatus.addIke.status}
      closeModal={() => changeModalStatus('addIke', false)}
    >
      <div className='header'>
        <div className='title'>Add IKE profile</div>
      </div>
      <div className='body'>
      ...
      <div className='footer'>
        <Button
          ...
          onClick={() => changeModalStatus('addIke', false)}
        />
