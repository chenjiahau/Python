...
import { getEditGreTunnel } from 'dummy/data/gateway/data/vpn/gre-tunnel/gre-tunnel-configuration';
...
const EditModal = ({
  ...
  useEffect(() => {
    const tmpGreTunnel = getEditGreTunnel(selectedGreTunnel);
    setGreTunnel(tmpGreTunnel);
  }, [modalStatus.editGreTunnel.status])

  if(!greTunnel) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.editGreTunnel.status}
      closeModal={() => changeModalStatus('editGreTunnel', false)}
    >
      <div className='header'>
        <div className='title'>Edit GRE tunnel</div>
      </div>
      <div className='body'>
      ...
      <div className='footer'>
        <Button
          ...
          onClick={() => changeModalStatus('editGreTunnel', false)}
        />
