...
import { getEditClientMode } from 'dummy/data/gateway/data/vpn/pptp-l2tp/client-mode';
...
const EditModal = ({
  ...
  useEffect(() => {
    const tmpClientMode = getEditClientMode(selectedClientMode);
    setClientMode(tmpClientMode);
  }, [modalStatus.editClientMode.status])

  if(!clientMode) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.editClientMode.status}
      closeModal={() => changeModalStatus('editClientMode', false)}
    >
      <div className='header'>
        <div className='title'>Edit PPTP/L2TP client</div>
      </div>
      <div className='body'>
...

        {
          ...
          <>
            <Button
              ...
              onClick={() => {
                changeModalStatus('editClientMode', false);
