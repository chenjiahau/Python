...
import { getEditOmniSslPortalLayoutConfiguration } from 'dummy/data/gateway/data/vpn/open-vpn/omni-ssl-portal-layout';
...
const EditModal = ({
  ...
  useEffect(() => {
    const tmpOmniSSLPortalLayout = getEditOmniSslPortalLayoutConfiguration(selectedOmniSSLPortalLayout);
    setOmniSSLPortalLayout(tmpOmniSSLPortalLayout);
  }, [modalStatus.editOmniSSLPortalLayout.status])

  if(!omniSSLPortalLayout) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.editOmniSSLPortalLayout.status}
      closeModal={() => changeModalStatus('editOmniSSLPortalLayout', false)}
    >
      ...
      <div className='footer'>
        <Button
          ...
          onClick={() => changeModalStatus('editOmniSSLPortalLayout', false)}
        />
        <Button
          label='Save'
