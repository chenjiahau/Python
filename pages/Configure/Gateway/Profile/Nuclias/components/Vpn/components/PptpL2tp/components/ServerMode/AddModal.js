...
import { getAddServerMode } from 'dummy/data/gateway/data/vpn/pptp-l2tp/server-mode';
...
const AddModal = ({
  ...
  useEffect(() => {
    const tmpIke = getAddServerMode();
    setServerMode(tmpIke);
  }, [modalStatus.addServerMode.status])

  if(!serverMode) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.addServerMode.status}
      closeModal={() => changeModalStatus('addServerMode', false)}
    >
      <div className='header'>
        <div className='title'>Add PPTP/L2TP server</div>
      </div>
      <div className='body'>
...

        {
          ...
          <>
            <Button
              ...
              onClick={() => {
                changeModalStatus('addServerMode', false);
