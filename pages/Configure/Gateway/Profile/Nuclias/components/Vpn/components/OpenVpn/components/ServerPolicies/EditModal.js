...
import { getEditServerPolicies } from 'dummy/data/gateway/data/vpn/open-vpn/server-policies';
...
const EditModal = ({
  ...
  useEffect(() => {
    const tmpServerPolicies = getEditServerPolicies(selectedServerPolicies);
    setServerPolicies(tmpServerPolicies);
  }, [modalStatus.editServerPolicies.status])

  if(!serverPolicies) return <></>
  return (
    <ModalContainer
      modalWidthType='modal-500px'
      openModal={modalStatus.editServerPolicies.status}
      closeModal={() => changeModalStatus('editServerPolicies', false)}
    >
      <div className='header'>
        <div className='title'>Edit OpenVPN server policy</div>
      </div>
      <div className='body'>
        <Row className='mt-3'>
          <Col sm={6}>
      ...
      <div className='footer'>
        <Button
          ...
          onClick={() => changeModalStatus('editServerPolicies', false)}
        />
        <Button
          label='Save'
