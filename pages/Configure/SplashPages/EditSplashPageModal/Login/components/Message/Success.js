...
const SuccessMessage = ({ isPreviewMode, isPhoneSize, selectedMessage }) => {
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.message.success}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.message.success}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.message.success}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.message.success}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-success-message-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-success-message-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-success-message-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.message.success}-text`, c.hex);
  }

  const changeFontSize = (e, value) => {
    setFSize(value);
    setFontSize(`${cpd.message.success}-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.message.success}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.message.success}-text`);
  ...
  useEffect(() => {
    if (selectedMessage && selectedMessage.position === 'success') {
  ...
  return (
    <div
      className={`sp-login-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.message.success}-container`}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''} sp-login-block--clear sp-login-block--no-border`}>
        <div className='sp-login-body-block sp-login-success-message '>
          <div
            className={`${cpd.message.success}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.message.success}-icon-block`}>
                <i
                  className={`${cpd.message.success}-edit-icon`}
        ...
        <div className={`${cpd.message.success}-edit-block`} onClick={stopPropagation}>
