...
const GetCodeButton = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('Get code');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.getCode}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.button.getCode}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.getCode}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.button.getCode}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-get-code-button-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-get-code-button-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-get-code-button-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeFontColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.button.getCode}-text`, c.hex);
  }

  const changeButtonColor = c => {
    setBtnColor(c.hex);
    setBackgroundColor(`${cpd.button.getCode}-text`, c.hex);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.button.getCode}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.button.getCode}-text`);
  ...
  return (
    <div
      className={`sp-login-button-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.button.getCode}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''} sp-login-block--clear`}>
        <div className='sp-login-full-body-block'>
          <div
            className={`sp-login-button ${cpd.button.getCode}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.button.getCode}-icon-block`}>
                <i
                  className={`${cpd.button.getCode}-edit-icon`}
        ...
        <div className={`${cpd.button.getCode}-edit-block`} onClick={stopPropagation}>
