...
const BackButton = ({ style, isPreviewMode, isPhoneSize }) => {
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.back}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.button.back}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.back}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.button.back}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-back-button-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-back-button-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-back-button-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeFontColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.button.back}-text`, c.hex);
  }

  const changeButtonColor = c => {
    setBtnColor(c.hex);
    setBackgroundColor(`${cpd.button.back}-text`, c.hex);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.button.back}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.button.back}-text`);
  ...
  return (
    <div
      className={`sp-login-button-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.button.back}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''} sp-login-block--clear`}>
        <div className='sp-login-full-body-block'>
          <div
            className={`sp-login-button ${cpd.button.back}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.button.back}-icon-block`}>
                <i
                  className={`${cpd.button.back}-edit-icon`}
        ...
        <div className={`${cpd.button.back}-edit-block`} onClick={stopPropagation}>
