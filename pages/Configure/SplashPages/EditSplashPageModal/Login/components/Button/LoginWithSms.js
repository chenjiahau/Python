...
const LoginWithSmsButton = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('Login with SMS');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.loginWithSms}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.button.loginWithSms}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.loginWithSms}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.button.loginWithSms}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-login-with-sms-button-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-login-with-sms-button-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-login-with-sms-button-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeFontColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.button.loginWithSms}-text`, c.hex);
  }

  const changeButtonColor = c => {
    setBtnColor(c.hex);
    setBackgroundColor(`${cpd.button.loginWithSms}-text`, c.hex);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.button.loginWithSms}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.button.loginWithSms}-text`);
  ...
  return (
    <div
      className={`sp-login-button-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.button.loginWithSms}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''} sp-login-block--clear`}>
        <div className='sp-login-full-body-block'>
          <div
            className={`sp-login-button ${cpd.button.loginWithSms}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.button.loginWithSms}-icon-block`}>
                <i
                  className={`${cpd.button.loginWithSms}-edit-icon`}
        ...
        <div className={`${cpd.button.loginWithSms}-edit-block`} onClick={stopPropagation}>
...
export default LoginWithSmsButton;