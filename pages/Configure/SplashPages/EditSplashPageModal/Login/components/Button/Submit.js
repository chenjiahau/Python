...
const SubmitButton = ({ style, isPreviewMode, isPhoneSize }) => {
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.submit}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.button.submit}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.button.submit}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.button.submit}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-submit-button-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-submit-button-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-submit-button-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeFontColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.button.submit}-text`, c.hex);
  }

  const changeButtonColor = c => {
    setBtnColor(c.hex);
    setBackgroundColor(`${cpd.button.submit}-text`, c.hex);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.button.submit}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.button.submit}-text`);
  ...
  return (
    <div
      className={`sp-login-button-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.button.submit}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''} sp-login-block--clear`}>
        <div className='sp-login-full-body-block'>
          <div
            className={`sp-login-button ${cpd.button.submit}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.button.submit}-icon-block`}>
                <i
                  className={`${cpd.button.submit}-edit-icon`}
        ...
        <div className={`${cpd.button.submit}-edit-block`} onClick={stopPropagation}>
