...
const SocialMediaTerms = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [byText, setByText] = useState('By logging in to my social media account for accessing Wi-Fi.');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.socialMedia}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.terms.socialMedia}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.socialMedia}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.terms.socialMedia}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-social-media-terms-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-social-media-terms-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-social-media-terms-edit-block', [isPhoneSize ? 'sp-login-phone-width-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);

    const doms = document.querySelectorAll(`.${cpd.terms.socialMedia}-text`);
  ...
  const changeFontSize = (e, value) => {
    ...
    setFontSize(`${cpd.terms.socialMedia}-link-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.terms.socialMedia}-text`, item.title);
    setFontFamily(`${cpd.terms.socialMedia}-link-text`, item.title);
  }
  ...
  return (
    <div
      className={`sp-login-text-container ${isPreviewMode ? 'sp-login-container--align-left-preview' : ''} ${cpd.terms.socialMedia}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview sp-login-block-terms-preview' : ''}`}>
        <div className='sp-login-body-block'>
          <span
            className={`sp-login-terms ${cpd.terms.socialMedia}-text`}
          ...
          <span className={`sp-login-terms ${cpd.terms.socialMedia}-link-text`}>
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.terms.socialMedia}-icon-block`}>
                <i
                  className={`${cpd.terms.socialMedia}-edit-icon`}
        ...
        <div className={`${cpd.terms.socialMedia}-edit-block`} onClick={stopPropagation}>
