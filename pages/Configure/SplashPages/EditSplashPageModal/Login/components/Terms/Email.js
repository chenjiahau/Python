...
const EmailTerms = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [byText, setByText] = useState('Sign up your e-mail to receive an authentication link for accessing Wi-Fi.');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.email}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.terms.email}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.email}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.terms.email}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-email-terms-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-email-terms-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-email-terms-edit-block', [isPhoneSize ? 'sp-login-phone-width-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);

    const doms = document.querySelectorAll(`.${cpd.terms.email}-text`);
  ...
  const changeFontSize = (e, value) => {
    ...
    setFontSize(`${cpd.terms.email}-link-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.terms.email}-text`, item.title);
    setFontFamily(`${cpd.terms.email}-link-text`, item.title);
  }
  ...
  return (
    <div
      className={`sp-login-text-container ${isPreviewMode ? 'sp-login-container--align-left-preview' : ''} ${cpd.terms.email}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview sp-login-block-terms-preview' : ''}`}>
        <div className='sp-login-body-block'>
          <div
            className={`sp-login-terms ${cpd.terms.email}-text`}
          ...
          <span className={`sp-login-terms ${cpd.terms.email}-link-text`}>
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.terms.email}-icon-block`}>
                <i
                  className={`${cpd.terms.email}-edit-icon`}
        ...
        <div className={`${cpd.terms.email}-edit-block`} onClick={stopPropagation}>
