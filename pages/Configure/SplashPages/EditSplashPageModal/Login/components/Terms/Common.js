...
const CommonTerms = ({ style, isPreviewMode, isPhoneSize }) => {
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.common}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.terms.common}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.common}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.terms.common}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-common-terms-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-common-terms-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-common-terms-edit-block', [isPhoneSize ? 'sp-login-phone-width-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.terms.common}-text`, c.hex);
  }

  const changeFontSize = (e, value) => {
    setFSize(value);
    setFontSize(`${cpd.terms.common}-text`, value);
    setFontSize(`${cpd.terms.common}-link-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.terms.common}-text`, item.title);
    setFontFamily(`${cpd.terms.common}-link-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.terms.common}-text`);
  ...
  return (
    <div
      className={`sp-login-text-container ${isPreviewMode ? 'sp-login-container--align-left-preview' : ''} ${cpd.terms.common}-container`}
      style={{ ...style }}
      onChange={() => { }}
      onMouseOver={showIconBlock}
      onMouseOut={hideIconBlock}
    >
      <div className={`sp-login-block sp-login-block-align-left ${isPreviewMode ? 'sp-login-block--preview sp-login-block-terms-preview' : ''}`}>
        <div className='sp-login-body-block'>
          <span
            className={`sp-login-terms ${cpd.terms.common}-text`}
          ...
          <span className={`sp-login-terms ${cpd.terms.common}-link-text`}>
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.terms.common}-icon-block`}>
                <i
                  className={`${cpd.terms.common}-edit-icon`}
        ...
        <div className={`${cpd.terms.common}-edit-block`} onClick={stopPropagation}>
