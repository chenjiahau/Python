...
const SmsTerms = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [byText, setByText] = useState('Request verification code through SMS to access the network.');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.sms}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.terms.sms}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.terms.sms}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.terms.sms}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-sms-terms-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-sms-terms-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-sms-terms-edit-block', [isPhoneSize ? 'sp-login-phone-width-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);

    const doms = document.querySelectorAll(`.${cpd.terms.sms}-text`);
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.terms.sms}-text`, item.title);
    setFontFamily(`${cpd.terms.sms}-link-text`, item.title);
  }
  ...
  return (
    <div
      className={`sp-login-text-container ${isPreviewMode ? 'sp-login-container--align-left-preview' : ''} ${cpd.terms.sms}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview sp-login-block-terms-preview' : ''}`}>
        <div className='sp-login-body-block'>
          <div
            className={`sp-login-terms ${cpd.terms.sms}-text`}
          ...
          <span className={`sp-login-terms ${cpd.terms.sms}-link-text`}>
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.terms.sms}-icon-block`}>
                <i
                  className={`${cpd.terms.sms}-edit-icon`}
        ...
        <div className={`${cpd.terms.sms}-edit-block`} onClick={stopPropagation}>
