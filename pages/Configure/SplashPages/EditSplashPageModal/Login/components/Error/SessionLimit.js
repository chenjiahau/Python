...
const SessionLimitError = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('You have reached the maximum number of permitted Internet sessions. Please close one or more applications to allow further Internet access. Contact the system administrator for more information.');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.error.sessionLimit}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.error.sessionLimit}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.error.sessionLimit}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.error.sessionLimit}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-session-limit-error-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-session-limit-error-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-session-limit-error-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setFontColor(`${cpd.error.sessionLimit}-text`, c.hex);

    const divDom = document.querySelectorAll(`.${cpd.error.sessionLimit}-text > div`);
  ...
  const changeFontSize = (e, value) => {
    setFSize(value);
    document.querySelector(`.${cpd.error.sessionLimit}-text`).style.fontSize = `${value}px`;
    const divDom = document.querySelectorAll(`.${cpd.error.sessionLimit}-text > div`);
  ...
  const changeFontFamily = item => {
    ...
    document.querySelector(`.${cpd.error.sessionLimit}-text`).style.fontFamily = item.title;
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.error.sessionLimit}-text`);
    const divDom = document.querySelectorAll(`.${cpd.error.sessionLimit}-text > div`);
  ...
  return (
    <div
      className={`sp-login-error-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.error.sessionLimit}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''}`}>
        <div className='sp-login-body-block'>
          <h1
            className={`sp-login-error ${cpd.error.sessionLimit}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.error.sessionLimit}-icon-block`}>
                <i
                  className={`${cpd.error.sessionLimit}-edit-icon`}
        ...
        <div className={`${cpd.error.sessionLimit}-edit-block`} onClick={stopPropagation}>
