...
const SessionLimitTitle = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('Max session limit reached');
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.sessionLimit}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.title.sessionLimit}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.sessionLimit}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.title.sessionLimit}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-session-limit-title-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-session-limit-title-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-session-limit-title-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.title.sessionLimit}-text`, c.hex);
  }

  const changeFontSize = (e, value) => {
    setFSize(value);
    setFontSize(`${cpd.title.sessionLimit}-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.title.sessionLimit}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.title.sessionLimit}-text`);
  ...
  return (
    <div
      className={`sp-login-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.title.sessionLimit}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''}`}>
        <div className='sp-login-body-block'>
          <h1
            className={`sp-login-title sp-login-title--small ${cpd.title.sessionLimit}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.title.sessionLimit}-icon-block`}>
                <i
                  className={`${cpd.title.sessionLimit}-edit-icon`}
        ...
        <div className={`${cpd.title.sessionLimit}-edit-block`} onClick={stopPropagation}>
                  ...
                  />
                  {
                    showFontColorPicker && (
                      <div>
                        ...
                        <SketchPicker onChange={changeColor} />
                      </div>
                    )
                  }
                </div>
...
export default SessionLimitTitle;