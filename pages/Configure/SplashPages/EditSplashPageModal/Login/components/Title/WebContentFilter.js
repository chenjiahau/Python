...
const WebContentFilterTitle = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('This content is blocked');
  const [showFontColorPicker, setShowFontColorPicker] = useState(false);

  const defaultFSize = 22;
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.webContentFilter}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.title.webContentFilter}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.webContentFilter}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.title.webContentFilter}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-web-content-filter-title-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-web-content-filter-title-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-web-content-filter-title-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.title.webContentFilter}-text`, c.hex);
  }

  const changeFontSize = (e, value) => {
    setFSize(value);
    setFontSize(`${cpd.title.webContentFilter}-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.title.webContentFilter}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.title.webContentFilter}-text`);
  ...
  return (
    <div
      className={`sp-login-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.title.webContentFilter}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''}`}>
        <div className='sp-login-body-block'>
          <h1
            className={`sp-login-title sp-login-title--small ${cpd.title.webContentFilter}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.title.webContentFilter}-icon-block`}>
                <i
                  className={`${cpd.title.webContentFilter}-edit-icon`}
        ...
        <div className={`${cpd.title.webContentFilter}-edit-block`} onClick={stopPropagation}>
...
export default WebContentFilterTitle;