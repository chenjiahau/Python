...
const WifiTermsAndConditionsTitle = ({ style, isPreviewMode, isPhoneSize }) => {

  // State
  const [text, setText] = useState('Wifi Terms and Conditions');
  const [showFontColorPicker, setShowFontColorPicker] = useState(false);

  const defaultFSize = 26;
  ...
  const showIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.wifiTermsAndConditions}-icon-block`, ['sp-login-visibility']);
    controlDom(`${cpd.title.wifiTermsAndConditions}-edit-icon`, ['sp-login-scale']);
  }

  const hideIconBlock = e => {
    stopPropagation(e);
    controlDom(`${cpd.title.wifiTermsAndConditions}-icon-block`, ['sp-login-visibility'], false);
    controlDom(`${cpd.title.wifiTermsAndConditions}-edit-icon`, ['sp-login-scale'], false);
  }
  ...
  const showEditBlock = e => {
    ...
    const containerDom = document.querySelector('.sp-login-wifi-terms-and-conditions-title-container');
    const height = containerDom.getBoundingClientRect().height;
    const editLogoDom = document.querySelector('.sp-login-wifi-terms-and-conditions-title-edit-block');

    containerDom.classList.add('sp-login-background');
    editLogoDom.style.top = `${height}px`;
    controlDom('sp-login-wifi-terms-and-conditions-title-edit-block', [isPhoneSize ? 'sp-login-phone-width' : 'sp-login-width', 'sp-login-opacity', 'sp-login-visibility']);
  }

  const changeColor = c => {
    setFColor(c.hex);
    setColor(`${cpd.title.wifiTermsAndConditions}-text`, c.hex);
  }

  const changeFontSize = (e, value) => {
    setFSize(value);
    setFontSize(`${cpd.title.wifiTermsAndConditions}-text`, value);
    showEditBlock(e);
  }
  ...
  const changeFontFamily = item => {
    ...
    setFontFamily(`${cpd.title.wifiTermsAndConditions}-text`, item.title);
  }

  const changeText = () => {
    const dom = document.querySelector(`.${cpd.title.wifiTermsAndConditions}-text`);
  ...
  return (
    <div
      className={`sp-login-container ${isPreviewMode ? 'sp-login-container--preview' : ''} ${cpd.title.wifiTermsAndConditions}-container`}
      style={{ ...style }}
      onChange={() => { }}
    ...
    >
      <div className={`sp-login-block ${isPreviewMode ? 'sp-login-block--preview' : ''}`}>
        <div className='sp-login-body-block'>
          <h1
            className={`sp-login-title sp-login-title--medium ${cpd.title.wifiTermsAndConditions}-text`}
        ...
        </div>
        {
          !isPreviewMode && (
            <>
              <div className={`${cpd.title.wifiTermsAndConditions}-icon-block`}>
                <i
                  className={`${cpd.title.wifiTermsAndConditions}-edit-icon`}
        ...
        <div className={`${cpd.title.wifiTermsAndConditions}-edit-block`} onClick={stopPropagation}>
...
export default WifiTermsAndConditionsTitle;