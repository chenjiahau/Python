...
const EditPortModal = props => {
  ...
  const renderPortMacAddress = () => {
    return selectedPortList.reduce((rows, key, index) => {
  ...
  useEffect(() => {
    ...
    if (selectedPortList.length === 1) {
      const port = cloneDeep(selectedPortList[0]);
    ...
    if (selectedPortList.length > 1) {
      ...
      // selectedPortList.forEach((port, index) => {
      //   console.log(port)
      // })
    }
  ...
  return (
    ...
    >
      <div className="header">
        <div className="title">
          Update {selectedPortList.length} {selectedPortList.length === 1 ? 'port' : 'ports'}
        </div>
      </div>
      <div className={`body ${mainStyle['edit-modal-body']}`}>
        <MessageBoxGroup
          messages={messages}
            ...
            <div className={mainStyle['port-mac-address-field']}>
              {
                renderPortMacAddress().map((port, index) => {
                  return (
                    ...
                    >
                      <div >Switch-{port[0].portGroup} / Port{port[0].port}</div>
                      {
                        port[1] && (
                          <div>Switch-{port[1].portGroup} / Port{port[1].port}</div>
                        )
                      }
                    </div>
                  );
          ...
          <Col sm={6}>
            {
              form.type.selected.title === 'Access' && (
                <>
                  ...
                  <div className="modal-form-field"><DropdownWithItem
            ...
            <>
              ...
              <Row className="mb-2">
                <Col>
                  ...
                  <div className="modal-form-field">
                    {
                      form.accessPolicy.selected.title === 'Port security delete-on-time mode' && (
                        <>
                          <div className={mainStyle['hint']}>
                            You can go to <span className={mainStyle['link']} onClick={redirectToSwitchPorts}>Switch Ports</span> to monitor dynamic whitelist MACs of each port with "Port security delete-on-time mode".
                          </div>
                        </>
                      )
                    }
                    {
                      form.accessPolicy.selected.title === 'Port security permanent mode' && (
                        <>
                          <div className={mainStyle['hint']}>
                            You can go to <span className={mainStyle['link']} onClick={redirectToSwitchPorts}>Switch Ports</span> to monitor dynamic whitelist MACs of each port with "Port security permanent mode".
                          </div>
                        </>
                      )
                    }
